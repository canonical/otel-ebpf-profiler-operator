...
class PyroscopeTesterCharm(ops.CharmBase):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        if not (config_path:=Path("/etc/pyroscope/config.yml")).exists():
            _runcmd(f"wget https://github.com/grafana/pyroscope/releases/download/v{PYRO_VERSION}/pyroscope_{PYRO_VERSION}_linux_amd64.deb")
        ...
        profiling = ProfilingEndpointProvider(self.model.relations.get('profiling', []), app=self.app)
        profiling.publish_endpoint(f"{own_IP}:8080")

        self.unit.status = ops.ActiveStatus(f"pyroscope {PYRO_VERSION} ready at http://{own_IP}:4040")
